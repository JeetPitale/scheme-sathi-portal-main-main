...
const SchemeDetail = () => {
    ...
    useEffect(() => {
        const fetchScheme = async () => {
            setLoading(true);
            const data = await SchemeService.getBySlug(slug);
            setScheme(data);
            setLoading(false);
        };
        if (slug) fetchScheme();
    ...
    const handleSubmit = async () => {
        ...
        if (response.success) {
            ...
            addNotification({
                ...
                message: `Your application for ${scheme.name} has been submitted successfully. Application ID: ${response.application.id}`,
                type: 'status',
                read: false,
            });
    ...
    return (<Layout>
        <div className="container py-6 md:py-10 max-w-3xl">
                    ...
                    <CardHeader>
                        <div className="flex items-center gap-2 mb-2 flex-wrap">
                            {scheme.government_level && (
                                <span className={`text-xs font-bold uppercase tracking-wider px-2.5 py-1 rounded-full ${scheme.government_level === 'Central'
                                    ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400'
                                    : 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'
                                    }`}>
                                    {scheme.government_level} {t('government')}
                                </span>
                            )}
                    ...
                    <CardContent className="space-y-6">
                            ...
                            <div className="space-y-4">
                                {benefits.financial_assistance && (
                                    <div>
                                        ...
                                        <p className="text-muted-foreground">{benefits.financial_assistance}</p>
                                    </div>
                                )}
                                {benefits.non_financial_support && (
                                    <div>
                                        <h3 className="font-semibold text-foreground mb-2 flex items-center gap-2">
                                            <Shield className="h-4 w-4 text-primary" /> {t('nonFinancialSupport')}
                                        </h3>
                                        <p className="text-muted-foreground">{benefits.non_financial_support}</p>
                                    </div>
                                )}
                            </div>
                        ) : (
                        ...
                        {scheme.application_process && (
                            <div>
                                ...
                                <p className="text-muted-foreground text-sm">{scheme.application_process}</p>
                            </div>
                        )}
                        ...
                        <div>
                            ...
                            <ul className="list-disc list-inside text-muted-foreground space-y-1">
                                ...
                                {documents.length === 0 && <li>{t('noDocumentsRequired')}</li>}
                            </ul>
                        </div>
            ...
            {/* Form, Documents, Confirm, Success steps remain same as ServiceDetail but using 'scheme' variable */}
            {step === 'form' && (
                <Card>
                    <CardHeader>
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground mt-1">{scheme.name}</p>
                            </div>
                        </div>
                    </CardHeader>
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {uploadedDocs.map((doc) => (
                                    <span key={doc} className="px-3 py-1 bg-accent/20 text-accent rounded-full text-sm flex items-center gap-1">
                                        ✓ {doc}
